...
const extractIncome = async (req, res) => {
  ...
  // On Render (production), Python OCR is unavailable — ask user to enter manually
  if (process.env.NODE_ENV === 'production') {
    return res.status(503).json({
      message: 'OCR service is not available in cloud deployment. Please enter the amount manually.',
    });
  }
