...
export default function Coverage() {
  const { policy, selectPlan, setPolicy } = usePolicy();
  const [selectedPlan, setSelectedPlan] = useState(null);

  // Auto-activate Basic plan if no policy exists
  useEffect(() => {
    if (!policy) {
      selectPlan('basic').then((result) => { if (setPolicy) setPolicy(result); }).catch(() => {});
    }
