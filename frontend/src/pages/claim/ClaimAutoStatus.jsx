...
export default function ClaimAutoStatus() {
  ...
  return (
    <div className="max-w-lg mx-auto space-y-6">
      ...
      {/* Payout breakdown */}
      {claim.payoutBreakdown && (
        <div className="card bg-blue-50 dark:bg-blue-900/20 border border-blue-100 dark:border-blue-800 space-y-2 text-sm">
          <p className="font-semibold text-blue-700 dark:text-blue-300 flex items-center gap-1.5">
            <Calculator size={14} /> Payout Calculation
          </p>
          {[
            ['Hourly Income',  formatCurrency(claim.payoutBreakdown.hourlyIncome)],
            ['Loss Hours',     `${claim.payoutBreakdown.lossHours} hrs`],
            ['Total Payout',   formatCurrency(claim.payoutBreakdown.totalPayout)],
          ].map(([label, value]) => (
            <div key={label} className="flex justify-between">
              <span className="text-gray-500">{label}</span>
          ...
          <p className="text-xs text-gray-400 pt-1 border-t border-blue-100 dark:border-blue-800">
            Formula: Hourly Income × {claim.payoutBreakdown.lossHours} hours
          </p>
        </div>
      )}
