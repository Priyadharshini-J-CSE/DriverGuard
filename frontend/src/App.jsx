...
import VerifyProcessing from './pages/claim/VerifyProcessing';
import ClaimSuccess from './pages/claim/ClaimSuccess';
import PayoutGateway from './pages/claim/PayoutGateway';
import PayoutProcessing from './pages/claim/PayoutProcessing';
import PayoutSuccess from './pages/claim/PayoutSuccess';
...
function PublicRoute({ children }) {
  const { user, loading } = useAuth();
  if (loading) return null; // login page can show spinner briefly
...
function RootRedirect() {
  const { user, loading } = useAuth();
  // Show landing page immediately — redirect only after auth resolves
...
function DisruptionWrapper() {
  const { user } = useAuth();
  const city = user?.city || 'Madurai';
  // Only provide disruption data when logged in
  if (!user) return <AppRoutes />;
  return (
    <DisruptionProvider city={city}>
      <AppRoutes />
    </DisruptionProvider>
  );
}
