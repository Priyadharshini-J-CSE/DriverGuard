...
const getLeaderboard = async (req, res, next) => {
  try {
    const users = await User.find({ role: 'worker' })
      .select('name city loyaltyPoints tier riskScore fraudCount')
...
/* PATCH /admin/policies/:id  — block the worker linked to a policy */
const blockPolicyUser = async (req, res, next) => {
  try {
    const policy = await Policy.findById(req.params.id);
    if (!policy) return res.status(404).json({ message: 'Policy not found' });
    await User.findByIdAndUpdate(policy.userId, { isBlocked: true });
...
/* PATCH /admin/policies/:id/unblock  — unblock the worker linked to a policy */
const unblockPolicyUser = async (req, res, next) => {
  try {
    const policy = await Policy.findById(req.params.id);
    if (!policy) return res.status(404).json({ message: 'Policy not found' });
    await User.findByIdAndUpdate(policy.userId, { isBlocked: false });
    const updated = await Policy.findByIdAndUpdate(
...
module.exports = {
  ...
  getFraudAlerts, getAnalytics, getAllUsers, updateUser, blockPolicyUser, unblockPolicyUser,
  getAllPayouts, getDisruptions, getSystemLogs, getTraffic,
  approveClaim, rejectClaim, initiatePayout, getLeaderboard,
};
