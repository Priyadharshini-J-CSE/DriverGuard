...
const FAQS = [
  { q: 'What does the insurance cover?',            a: 'We cover income loss caused by heavy rain, extreme heat, poor air quality, severe traffic jams, government lockdowns, and public strikes or bandhs — anything that stops you from working through no fault of your own.' },
  { q: 'When will I receive my payout?',             a: 'Once a disruption is confirmed, your payout is processed automatically within 24 hours directly to your registered bank account. No waiting, no follow-ups needed.' },
  { q: 'How much does it cost?',                     a: 'Plans start at just 5% of your weekly income. You only pay a small premium and get covered for the whole week. The more you earn, the more you are protected.' },
  { q: 'Do I need to submit any documents?',         a: 'Only once during registration — a valid ID and a recent screenshot or PDF of your earnings from your delivery app. After that, everything is automatic.' },
  { q: 'Can I change or cancel my plan?',            a: 'Yes. You can upgrade, downgrade, or cancel your plan at the start of any new weekly cycle. There are no lock-in periods or cancellation fees.' },
  { q: 'What if I choose not to work on a bad day?', a: 'Payouts are only triggered by verified external disruptions. If conditions are safe but you choose not to work, that day is not covered — this keeps the system fair for everyone.' },
  { q: 'Is my personal data safe?',                  a: 'Absolutely. All your personal and financial information is encrypted and stored securely. We never share your data with third parties or your delivery platform.' },
  { q: 'Do I need to file a claim manually?',        a: 'No. Our system monitors conditions around the clock. When a disruption crosses the threshold, your claim is raised and paid out automatically — no forms, no calls required.' },
];
...
function FAQItem({ q, a }) {
  ...
  return (
    <div className="bg-white rounded-2xl overflow-hidden shadow-sm border border-indigo-50 transition-shadow hover:shadow-md">
      <button
        onClick={() => setOpen(!open)}
        className="w-full flex items-center justify-between px-6 py-4 text-left font-semibold text-gray-800 hover:bg-indigo-50/40 transition-colors"
      >
        <span className="pr-4 text-sm">{q}</span>
        <ChevronDown size={18} className={`text-indigo-400 flex-shrink-0 transition-transform duration-300 ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && (
        <div className="px-6 pb-5 text-sm text-gray-500 leading-relaxed border-t border-indigo-50 pt-3">
          {a}
        </div>
      )}
    </div>
...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-white text-gray-900 font-sans">
      ...
      {/* ── SECTION 8: FAQ ── */}
      <section className="py-20 px-6" style={{ background: '#f0f4ff' }}>
        <div className="max-w-2xl mx-auto">
          <div className="text-center mb-12">
            <span className="inline-block text-xs font-semibold tracking-widest uppercase text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full mb-3">FAQ</span>
